...
SilverWolfe Application Development ("we," "us," or "our") operates the Library Cat Scanner mobile application (the "Service"). This page informs you of our policies regarding the collection, use, and disclosure of personal data when you use our Service.
1. Data Collection and Use
We believe in user privacy. Because Library Cat Scanner is a stand-alone application, we do not collect, store, or transmit any personally identifiable information (PII) to external servers.
...
2. Permissions
The app may request certain permissions to function correctly (e.g., Camera, Photo Library, or Notifications). These permissions are used strictly for the app’s core functionality:
•	Camera: Used only for scanning books for cataloging the user’s personal library
•	This data can be uploaded and the data files can be shared, but the data is publicly available data.
